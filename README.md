...
- 🏝️ **Island Architecture**: Built on Fresh, zero client-side JavaScript, with
  on-demand hydration.
- 📝 **Markdown Support**: Powerful Markdown processing capabilities via a
  dedicated plugin.
...
We are actively working on expanding the FreshPress ecosystem. Here are some of
the features and plugins planned for future releases:

- **Sitemap Generation**: Automatic `sitemap.xml` generation for better SEO.
- **`@freshpress/plugin-search`**: A dependency-free, client-side full-text
  search plugin.
- **`@freshpress/plugin-i18n`**: A plugin for multi-language content routing and
